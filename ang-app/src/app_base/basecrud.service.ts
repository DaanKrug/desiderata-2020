...
export class BaseCrudService {
    ...
    meta: Meta;
    getsntptimestamp: boolean;
    forceDropOnDelete: boolean;
	...
	// Do not override!
	setUseSNTP(useSNTP: boolean){
		this.getsntptimestamp = useSNTP;
	}
	
	// Do not override!
	setForceDropOnDelete(forceDropOnDelete: boolean){
		this.forceDropOnDelete = forceDropOnDelete;
	}
	...
	protected setHeaders(){
		var csrf = null != this.storageService 
		         ? this.storageService.localStorageGetItem('_token_' + this.getAppId()) 
		         : '';
	...
	protected stringify(object){
		object.getsntptimestamp = this.getsntptimestamp;
	...
	getBase(){
		...
		if(mockForRemote == 'true'){
			return 'https://desiderata.skallerten.com.br/desiderata/';
		}
	...
	drop(id): Promise<ValidationResult>{
		if(this.forceDropOnDelete){
			return this.trullyDrop(id);
		}
		this.setInActivity();
